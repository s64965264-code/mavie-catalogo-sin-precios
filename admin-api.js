(() => {
  ...
  function renderAdminProducts() {
    ...
    content.innerHTML = `<p class="kicker">MAVIE BEAUTY</p><h2 id="adminTitle">Productos</h2>
      <p class="admin-copy">Administra únicamente los productos de este catálogo.</p>
      <div class="admin-product-list">${available.length ? available.map((card) => {
        const page = Number(card.dataset.page);
        ...
        return `<div class="admin-product-row"><span><small>${String(page).padStart(2, "0")}</small>${title}</span>
          <button type="button" data-admin-delete="${page}">Eliminar</button></div>`;
      }).join("") : '<p class="admin-copy">No hay productos disponibles en este catálogo.</p>'}</div>`;
    ...
    events.onmessage = (event) => {
      try {
        ...
        if (data.catalog === catalog) {
          deletedPages.add(Number(data.page));
          applyProductVisibility();
          if (adminToken) renderAdminProducts();
        }
      } catch {}
